...
setup_database() {
    ...
    if [[ -f "backend/setup-database.sql" ]]; then
        if [[ "$(detect_os)" == "macos" ]]; then
            # On macOS, Homebrew Postgres usually trusts local connections
            psql -h localhost -U qenergy_user -d qenergy_platform -f backend/setup-database.sql
        else
            # On Linux, run as postgres superuser to avoid password prompts and ensure extensions can be created
            sudo -u postgres psql -d qenergy_platform -f backend/setup-database.sql
        fi
...
setup_frontend() {
    ...
    # Detect server IP or use PUBLIC_SERVER_IP override
    OS=$(detect_os)
    ...
    if [[ -z "$SERVER_IP" ]]; then
        print_warning "Could not auto-detect server IP. Falling back to localhost for NEXT_PUBLIC_API_URL. Set PUBLIC_SERVER_IP to override."
        API_BASE_URL="http://localhost:8002/api"
    else
        API_BASE_URL="http://$SERVER_IP:8002/api"
    fi

    cat > .env.local << EOF
NEXT_PUBLIC_API_URL=$API_BASE_URL
EOF
    print_success "Frontend environment file configured (NEXT_PUBLIC_API_URL=$API_BASE_URL)"
